...
import { encryptSecret } from "@/lib/comments-s2s-crypto";
...
export async function POST(request: Request) {
  ...
  const consent = await prisma.commentsIntegrationConsent.findUnique({
  ...
  const apiKey = `comments_live_${randomBytes(24).toString("base64url")}`;
  const secret = randomBytes(32).toString("base64url");
  const secretCiphertext = encryptSecret(secret);

  await prisma.$transaction([
    prisma.commentsIntegrationConsent.update({
      where: { id: consent.id },
